...
surat=$(curl -sS https://raw.githubusercontent.com/ryz-code/izin-vps/master/bot | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/ryz-code/bot-panel/master/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
